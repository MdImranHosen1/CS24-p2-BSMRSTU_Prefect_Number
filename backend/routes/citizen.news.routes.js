...
const citizenNews = require('../model/citizen.news.model')
...
router.post('/',async (req,res)=>{
    try {
        ...
        const citizenNews = new citizenNews(req.body)
        await citizenNews.save()
        res.status(201).json(citizenNews)
      } catch (err) {
        console.log(err)
        res.status(404).json('News Not Created')
