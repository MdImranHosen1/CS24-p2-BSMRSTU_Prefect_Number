...
const WorkforceTrack = require('../model/workforce.tracking.model')
...
router.get('/', async (req, res) => {
    try {
        ...
        const employees = await WorkforceTrack.find();
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const employee = await WorkforceTrack.findById(employeeId);
...
router.post('/', async (req, res) => {
    try {
        const { employeeId, servingWard, fullName, employeeType } = req.body;

        // Create a new employee document
        const newEmployee = new WorkforceTrack({
            employeeId,
            servingWard,
            fullName,
            employeeType,
...
// Route to update employee's service profile for a day
router.post('/:id/service', async (req, res) => {
    try {
        const employeeId = req.params.id;
        const { day, housesServed } = req.body;
        ...
        // Update the service profile for the day
        employee.serviceThisMonth.push({ day, housesServed });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const deletedEmployee = await WorkforceTrack.findByIdAndDelete(employeeId);
